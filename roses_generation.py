...
def generate_student_record(fake: Faker) -> tuple:
    ...
    leaves_length = random.randint(5, 15)
    pinnation = random.randint(3, 13)
    thorns = fake.boolean()
    company_producer = fake.company()
    petal_size = random.randint(2, 7)
    return (name, color, leaves_length, pinnation, thorns, company_producer, petal_size)
...
def insert_students(
    ...
    insert_query = """
        INSERT INTO roses
        (name, color, leaves_length, pinnation, thorns, company_producer, petal_size)
